...
function it_exchange_category_related_posts() {
	...
	*/
	$current_exchange_product = $GLOBALS['it_exchange']['product'];
	...
	if ( !empty($settings['width-int']) && !empty($settings['width-unit']) ) {
	...
	$args = array(
		...
		'orderby'      => 'rand',
		'post__not_in' => array( $current_exchange_product->ID ),
		'tax_query'    => array(
			array(
...
?>
	...
	<?php if ( !empty( $products ) ) { ?>
		...
		<h3 class="it-exchange-category-related-products-heading">
			<?php $heading = apply_filters( 'it_exchange_category_related_product_heading', __( 'Related Products', 'category-related-products-for-ithemes-exchange' ) ); ?>
			<?php echo $heading; ?>
		</h3>
		...
		<?php foreach ( $products as $product ) { ?>
			...
			<?php $exchange_product = it_exchange_get_product( $product->ID ); ?>
		...
		<?php $GLOBALS['it_exchange']['product'] = $current_exchange_product; ?>
	<?php } ?>
